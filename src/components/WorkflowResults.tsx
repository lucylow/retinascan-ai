  ...
  if (workflow.status === 'failed') {
    return (
      <div className="results-error">
        ...
        <button className="retry-button">Retry Analysis</button>
      </div>
    );
  }

  if (workflow.status === 'completed' && workflow.result) {
    const report = workflow.result.final_report || {};
    const diagnosis = report.diagnostic_findings || {};
    const clinical = report.clinical_assessment || { risk_factors: [] };
    const recommendations = report.recommendations || { immediate_actions: [] };
    ...
    return (
      <div className="results-completed">
        ...
        <div className="diagnosis-summary">
          <div className={`diagnosis-card ${clinical.urgency_level}`}>
            <div className="diagnosis-main">
              <h4>{diagnosis.primary_diagnosis || 'Unknown'}</h4>
              <div className="confidence-badge">
                Confidence: {((diagnosis.confidence_score || 0) * 100).toFixed(1)}%
              </div>
            </div>
            <div className="urgency-indicator">
              <span className={`urgency-badge ${clinical.urgency_level || 'low'}`}>
                {(clinical.urgency_level || 'low').toString().toUpperCase()} PRIORITY
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="detail-section">
            ...
            <div className="detail-grid">
              <div className="detail-item">
                ...
                <span className="detail-value">{(diagnosis.severity_level ?? '-')}/4</span>
              </div>
              <div className="detail-item">
                <span className="detail-label">Quality Score</span>
                <span className="detail-value">{clinical.quality_assessment || '-'}</span>
              </div>
              <div className="detail-item">
                <span className="detail-label">Recommended Follow-up</span>
                <span className="detail-value">{recommendations.follow_up_timeline || '-'}</span>
              </div>
            </div>
          </div>
        ...
        <div className="technical-details">
          ...
          <div className="tech-grid">
            ...
            <div className="tech-item">
              ...
              <span>{((workflow.result.processing_time || 0)).toFixed ? (workflow.result.processing_time || 0).toFixed(2) + 's' : '-'}</span>
            </div>
            <div className="tech-item">
